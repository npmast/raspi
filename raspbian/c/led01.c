...
int main(void)
{
    ...
    unsigned int lines[] = {14};             // ← 반드시 배열/변수

    chip = gpiod_chip_open(CHIP);            // CHIP 을 열어 디스크립트를 얻는다(GPIO 드라이버 연결)

    settings = gpiod_line_settings_new();    // 빈 객체 생성
    gpiod_line_settings_set_direction(settings, GPIOD_LINE_DIRECTION_OUTPUT);    // 출력 모드 설정

    config = gpiod_line_config_new();      // 빈 객체 생성
    gpiod_line_config_add_line_settings(config, lines, 1, settings);   // 14번핀 settings 적용

    req_cfg = gpiod_request_config_new();    // 요청할 GPIO 객체 생성
    gpiod_request_config_set_consumer(req_cfg, "led");

    req = gpiod_chip_request_lines(chip, req_cfg, config);  // GPIO 14을 현재 프로세스가 점유
